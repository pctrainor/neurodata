...
const categoryColors: Record<string, { 
  ...
  dataAnalyzer: {
    ...
    bg: 'bg-gradient-to-r from-purple-500/20 via-violet-500/20 to-fuchsia-500/20',
...
export interface DataNodeData {
  ...
  inputText?: string  // User-typed input text/prompt
  textContent?: string  // Legacy field for text content
  // User uploaded dataset fields
...
export const BrainNode = memo(({ data, selected, isRunning }: NodeProps & { isRunning?: boolean }) => {
  ...
  // Determine if this is the AI Assistant (general mode) or Brain Orchestrator
  const isAIAssistant = nodeData.mode === 'general'
  const colorKey = isAIAssistant ? 'aiAssistant' : 'brain'
  const IconComponent = isAIAssistant ? Sparkles : Brain
  const iconColor = isAIAssistant ? 'text-indigo-400' : 'text-pink-400'
  const bgColor = isAIAssistant ? 'bg-indigo-500/20' : 'bg-pink-500/20'
  const handleColor = isAIAssistant ? '!bg-indigo-500' : '!bg-pink-500'
  const subtitleText = isAIAssistant ? 'AI Assistant' : 'AI Orchestrator'
  ...
  return (
    <Tooltip
      content={
        <div className={cn(
          ...
          <div className={cn("text-[10px]", isAIAssistant ? "text-indigo-400" : "text-pink-400")}>{subtitleText}</div>
          {role && (
            <div className={cn("text-[10px]", isAIAssistant ? "text-indigo-400" : "text-pink-400")}>Role: {role}</div>
          )}
          ...
          {isAiGenerated && !demographics && (
            <div className={cn("text-[10px] italic", isAIAssistant ? "text-indigo-400" : "text-pink-400")}>
    ...
    >
      ...
      >
        <div className={cn("p-1 rounded-xl", bgColor)}>
          <IconComponent className={cn("w-3.5 h-3.5", iconColor)} />
        </div>
        <Handle 
          type="target" 
          position={Position.Left} 
          className={cn("!w-2 !h-2 !border !border-slate-900", handleColor)} 
        />
        <Handle 
          type="source" 
          position={Position.Right} 
...
export const DataNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  // Check if this is a user-uploaded dataset
  const isUserDataset = nodeData.isUserDataset === true
  ...
  // Use Database icon for user datasets, otherwise use mapped icon
  const Icon = isUserDataset ? Database : (iconMap[nodeData.subType] || Database)
  ...
  // Generate subtitle for user datasets with formatted numbers
  const getSubtitle = () => {
    if (isUserDataset) {
      const rows = typeof nodeData.rowCount === 'number' ? nodeData.rowCount.toLocaleString() : '?'
      const cols = nodeData.columns?.length || 0
      return `${rows} rows • ${cols} cols`
    }
    return subtitleMap[nodeData.subType] || 'Data Source'
  }
  ...
  // Handle file drop OR user dataset drop from palette
  const handleDrop = useCallback((e: React.DragEvent) => {
    e.preventDefault()
    e.stopPropagation()
    setIsDragOver(false)
    
    // Check if this is a user dataset drag from the palette
    const reactFlowData = e.dataTransfer.getData('application/reactflow')
    const payloadData = e.dataTransfer.getData('application/payload')
    
    if (reactFlowData === 'dataNode' && payloadData) {
      try {
        const payload = JSON.parse(payloadData)
        
        // Check if this is a user dataset being dropped
        if (payload.isUserDataset) {
          console.log('[DataNode] User dataset dropped onto node:', payload)
          
          // Update this node with the user dataset info
          setNodes((nodes) =>
            nodes.map((node) =>
              node.id === id
                ? {
                    ...
                    data: {
                      ...
                      sampleDataDescription: `${payload.rowCount || '?'} rows from ${payload.fileName}`,
                      aiGenerated: false,
                    },
                  }
                : node
          ...
          setLoadedFile({ name: payload.fileName, size: 0 })
          return
        }
      } catch (err) {
        console.error('[DataNode] Failed to parse payload:', err)
      }
    }
  ...
  useEffect(() => {
    if (fileName && !loadedFile) {
      // For user datasets, use fileSize from nodeData; for regular file drops, it was set during drop
      const size = nodeData.fileSize as number || 0
      setLoadedFile({ name: fileName, size })
    }
  }, [fileName, loadedFile, nodeData.fileSize])
  
  // For user datasets, always set the file info from nodeData
  useEffect(() => {
    if (isUserDataset && nodeData.fileName && !loadedFile) {
      setLoadedFile({ 
        name: nodeData.fileName as string, 
        size: (nodeData.fileSize as number) || 0 
      })
    }
  }, [isUserDataset, nodeData.fileName, nodeData.fileSize, loadedFile])
      ...
      content={
        <div className={cn(
          ...
          <div className="flex items-center gap-1.5">
            ...
            {isUserDataset && (
              <span className="flex items-center gap-0.5 px-1 py-0.5 rounded bg-blue-500/20 text-[8px] font-bold text-blue-400">
                📊 Library Dataset
              </span>
            )}
            <span className="font-bold text-xs">{nodeData.label}</span>
          ...
          {/* Dataset metadata for library datasets */}
          {isUserDataset && (
            <div className="text-[10px] space-y-1 border-t border-slate-600 pt-1.5 mt-1">
              <div className="flex justify-between">
                <span className="text-slate-400">Rows:</span>
                <span className="text-emerald-400 font-medium">{(nodeData.rowCount as number)?.toLocaleString() || '?'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Columns:</span>
                <span className="text-emerald-400 font-medium">{nodeData.columns?.length || '?'}</span>
              </div>
              {typeof nodeData.fileSizeMb === 'number' && nodeData.fileSizeMb > 0 && (
                <div className="flex justify-between">
                  <span className="text-slate-400">Size:</span>
                  <span className="text-emerald-400 font-medium">
                    {nodeData.fileSizeMb >= 1 
                      ? `${nodeData.fileSizeMb.toFixed(1)} MB` 
                      : `${(nodeData.fileSizeMb * 1024).toFixed(0)} KB`}
                  </span>
                </div>
              )}
              {/* Column names */}
              {nodeData.columns && nodeData.columns.length > 0 && (
                <div className="mt-2">
                  <span className="text-slate-400 text-[9px] uppercase tracking-wider">Columns:</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {(nodeData.columns as Array<{name: string}>).slice(0, 8).map((col, i) => (
                      <span key={i} className="px-1.5 py-0.5 bg-slate-700/50 rounded text-[9px] text-cyan-300">
                        {col.name}
                      </span>
                    ))}
                    {(nodeData.columns as Array<{name: string}>).length > 8 && (
                      <span className="text-[9px] text-slate-500">+{(nodeData.columns as Array<{name: string}>).length - 8} more</span>
                    )}
                  </div>
                </div>
              )}
              {/* Sample data preview */}
              {(nodeData.sampleRows as Array<Record<string, string>>)?.length > 0 && (
                <div className="mt-2">
                  <span className="text-slate-400 text-[9px] uppercase tracking-wider">Preview (first 3 rows):</span>
                  <div className="mt-1 overflow-x-auto">
                    <table className="text-[8px] w-full">
                      <tbody>
                        {(nodeData.sampleRows as Array<Record<string, string>>).slice(0, 3).map((row, i) => (
                          <tr key={i} className="border-b border-slate-700/50">
                            {Object.values(row).slice(0, 4).map((val, j) => (
                              <td key={j} className="px-1 py-0.5 text-slate-300 truncate max-w-[60px]">
                                {String(val).slice(0, 12)}{String(val).length > 12 ? '...' : ''}
                              </td>
                            ))}
                          </tr>
                        ))}
              ...
              <div className="text-[9px] text-slate-500 italic mt-1">
                🔗 Data stored in Supabase Storage
              </div>
            </div>
          )}
          
          <div className="text-[10px] text-slate-400">{!isUserDataset && (subtitleMap[nodeData.subType] || 'Data Source')}</div>
          {loadedFile && (
          ...
          {hasInputText && (
            <div className="text-[10px] text-cyan-400 border-t border-slate-600 pt-1.5 mt-1">
              <span className="font-semibold">✏️ Input Text:</span>
              <div className="text-slate-300 mt-0.5 max-h-16 overflow-hidden">
          ...
          {isAiGenerated && !sampleDataDescription && !loadedFile && !hasInputText && (
            <div className="text-[10px] text-purple-400 italic">
              ✦ Generated by AI Wizard
            </div>
          )}
          {!loadedFile && !hasInputText && (
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          // Only apply nodrag if this is NOT a user dataset - user datasets should be draggable
          // The nodrag class lets us handle drops but prevents node movement
          !isUserDataset && 'nodrag',
      ...
      >
        <NodeWrapper 
          ...
          subtitle={loadedFile ? 'File Loaded ✓' : (hasInputText ? 'Input Text ✓' : (isUserDataset ? getSubtitle() : (sampleDataDescription ? 'Data Configured' : (subtitleMap[nodeData.subType] || 'Data Source'))))}
          aiGenerated={isAiGenerated}
        >
          {/* User dataset badge */}
          {isUserDataset && (
            <div className="absolute -top-1 -right-1 px-1 py-0.5 rounded bg-blue-500 text-[6px] font-bold text-white z-10">
              YOUR DATA
            </div>
          )}
          {/* Input text badge */}
          {hasInputText && !isUserDataset && !loadedFile && (
            <div className="absolute -top-1 -right-1 px-1 py-0.5 rounded bg-cyan-500 text-[6px] font-bold text-white z-10">
              TEXT
            </div>
          )}
          {/* Drop zone indicator */}
          ...
          <div className={cn(
            ...
            isUserDataset ? "bg-blue-500/30" : (loadedFile ? "bg-emerald-500/30" : (hasInputText ? "bg-cyan-500/30" : (sampleDataDescription ? "bg-emerald-500/25" : "bg-emerald-500/20")))
          )}>
            {loadedFile ? (
          ...
          <Handle 
            ...
            position={Position.Right} 
            className={cn(
              "!w-2 !h-2 !border !border-slate-900",
              hasInputText ? "!bg-cyan-500" : (isUserDataset ? "!bg-blue-500" : "!bg-emerald-500")
            )}
          />
        </NodeWrapper>
        
        {/* Show input text preview below the node when configured */}
        {hasInputText && !loadedFile && !isUserDataset && (
          <div className="absolute -bottom-5 left-0 right-0 text-center">
            <span className="text-[7px] text-cyan-400 truncate block px-1">
              ✏️ {inputText!.length > 20 ? inputText!.slice(0, 20) + '...' : inputText}
            </span>
          </div>
        )}
        
        {/* Show sample data description below the node when configured (NOT for user datasets - they have their own display) */}
        {sampleDataDescription && !loadedFile && !hasInputText && !isUserDataset && (
          <div className="absolute -bottom-5 left-0 right-0 text-center">
        ...
        {/* Show row/col info for library datasets - THIS IS THE ONLY TEXT FOR USER DATASETS */}
        {isUserDataset && (
          <div className="absolute -bottom-5 left-0 right-0 text-center">
            <span className="text-[7px] text-blue-400 truncate block px-1">
              {typeof nodeData.rowCount === 'number' ? nodeData.rowCount.toLocaleString() : '?'} rows • {nodeData.columns?.length || '?'} cols
            </span>
          </div>
        )}
      </div>
...
// --- 13. AI Data Analyst Node (Server-Side Stats + AI Insights for Large Datasets) ---

// AI Insights structure from Gemini
...
export interface DataAnalyzerNodeData {
  label: string
  analysisType: 'full' | 'sample' | 'schema-only'
  status?: NodeStatus
  progress?: number
  ...
  stats?: {
    ...
    anomalies?: Array<{ row: number; column: string; reason: string }>
    summary?: string
...
export const DataAnalyzerNode = memo(({ data, selected }: NodeProps) => {
  const nodeData = data as DataAnalyzerNodeData
  const { resolvedTheme } = useTheme()
  const isDark = resolvedTheme === 'dark'
  ...
  return (
    <Tooltip
      content={
        <div className={cn(
          'space-y-2 max-w-[300px]',
          isDark ? 'text-slate-100' : 'text-slate-800'
        )}>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-gradient-to-r from-purple-500/20 via-violet-500/20 to-fuchsia-500/20 text-[9px] font-bold text-purple-400">
          ...
          <p className="text-[10px] text-slate-400">
            ✨ <span className="text-purple-300">AI-powered analysis</span> of entire datasets. Gemini analyzes patterns, correlations, anomalies, and generates intelligent insights.
          </p>
          
          {stats ? (
            <div className="space-y-2 border-t border-purple-500/30 pt-2">
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <div>
                  <span className="text-slate-400">Rows:</span>
                  <span className="ml-1 text-purple-400 font-medium">{stats.rowCount.toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-slate-400">Columns:</span>
                  <span className="ml-1 text-purple-400 font-medium">{stats.columnCount}</span>
                </div>
              </div>
              
              {stats.columns && stats.columns.length > 0 && (
                <div className="text-[9px]">
                  <span className="text-slate-400 uppercase tracking-wider">Column Stats:</span>
                  <div className="mt-1 space-y-1 max-h-32 overflow-y-auto">
                    {stats.columns.slice(0, 5).map((col, i) => (
                      <div key={i} className="flex justify-between items-center bg-purple-900/30 px-1.5 py-0.5 rounded">
                        <span className="text-violet-300 truncate max-w-[80px]">{col.name}</span>
                        <span className="text-slate-400 text-[8px]">
                          {col.type} • {col.uniqueCount?.toLocaleString()} unique
                        </span>
                      </div>
                    ))}
                    {stats.columns.length > 5 && (
                      <span className="text-slate-500 text-[8px]">+{stats.columns.length - 5} more columns</span>
                    )}
                  </div>
                </div>
              )}
              ...
              {stats.aiInsights && (
                <div className="mt-2 border-t border-purple-500/30 pt-2">
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-[10px] font-semibold text-purple-300">AI INSIGHTS</span>
                    {stats.aiInsights.dataQuality && (
                      <span className={cn(
                        "ml-auto px-1.5 py-0.5 rounded text-[8px] font-bold",
                        stats.aiInsights.dataQuality.score >= 80 ? "bg-green-500/20 text-green-400" :
                        stats.aiInsights.dataQuality.score >= 60 ? "bg-yellow-500/20 text-yellow-400" :
                        "bg-red-500/20 text-red-400"
                      )}>
                        Quality: {stats.aiInsights.dataQuality.score}/100
                      </span>
                    )}
                  </div>
                  
                  {/* Key Findings */}
                  {stats.aiInsights.keyFindings && stats.aiInsights.keyFindings.length > 0 && (
                    <div className="text-[9px] space-y-0.5 mb-1">
                      {stats.aiInsights.keyFindings.slice(0, 2).map((finding, i) => (
                        <div key={i} className="flex gap-1 text-slate-300">
                  ...
                  {/* Anomalies Warning */}
                  {stats.aiInsights.anomalies && stats.aiInsights.anomalies.length > 0 && (
                    <div className="text-[8px] text-amber-400 bg-amber-500/10 px-1 py-0.5 rounded">
                      ⚠️ {stats.aiInsights.anomalies.length} anomalies detected
                    </div>
                  )}
                  
                  {/* Suggested Questions Preview */}
                  {stats.aiInsights.suggestedQuestions && stats.aiInsights.suggestedQuestions.length > 0 && (
                    <div className="text-[8px] text-slate-400 mt-1">
                      💡 Ask: "{stats.aiInsights.suggestedQuestions[0].slice(0, 50)}..."
                    </div>
                  )}
                </div>
              )}
              
              {stats.summary && !stats.aiInsights && (
                <div className="text-[9px] text-teal-300 italic border-t border-slate-700 pt-1">
                  {stats.summary}
                </div>
              )}
            </div>
          ) : (
            <div className="text-[10px] text-slate-500 border-t border-slate-600 pt-2">
              Connect a dataset and run the workflow to analyze.
              <br />
              <span className="text-purple-400">✨ AI-powered pattern detection</span>
              <br />
              <span className="text-purple-400">✨ Handles millions of rows</span>
              <br />
              <span className="text-purple-400">✨ Feeds insights to AI Assistant</span>
            </div>
          )}
        </div>
      }
    ...
    >
      <NodeWrapper 
        selected={selected} 
        colorKey="dataAnalyzer" 
        label={nodeData.label}
        subtitle={stats?.aiInsights ? `Quality: ${stats.aiInsights.dataQuality.score}/100` : stats ? `${stats.rowCount.toLocaleString()} rows analyzed` : (analysisLabels[nodeData.analysisType] || 'AI Analyst')}
        status={nodeData.status}
        progress={nodeData.progress}
      >
        <div className="p-1 bg-gradient-to-br from-purple-500/20 via-violet-500/20 to-fuchsia-500/20 rounded-xl relative">
          <Sparkles className="w-3.5 h-3.5 text-purple-400" />
          {/* Subtle glow effect */}
          <div className="absolute inset-0 bg-gradient-to-br from-purple-400/10 to-fuchsia-400/10 rounded-xl animate-pulse" style={{ animationDuration: '3s' }} />
        </div>
      </NodeWrapper>
      {/* Handles must be direct children of the node root for proper React Flow detection */}
      <Handle 
        type="target" 
        position={Position.Left} 
...
DataAnalyzerNode.displayName = 'DataAnalyzerNode'
...
export const nodeTypes = {
  ...
  dataAnalyzerNode: DataAnalyzerNode,
}
