...
import { 
  Play, Youtube, Edit3, ExternalLink, Loader2, Brain, FileText, 
  Radio, Database, Globe, Activity, Library, Zap, BarChart3, 
  Sparkles, FileOutput, Cpu, MapPin, ChevronDown, Search, Video, Scale
} from 'lucide-react';
...
// Stub for useTheme (resolvedTheme defaults to 'dark')
function useTheme() {
  return { resolvedTheme: 'dark' };
}

// Local type definitions (avoiding external dependency issues)
    ...
    <Tooltip
      content={
        <div className={cn(
          ...
          <div className="font-bold text-xs">{nodeData.label}</div>
          <div className="text-[10px]">AI Orchestrator</div>
          ...
          {nodeResult ? (
            <div>
              <div className="font-semibold text-xs mt-1">Engagement: {nodeResult.engagement}</div>
            ...
            <div className="text-xs text-slate-400">No result yet</div>
          )}
        </div>
      }
    >
...
export const DataNode = memo(({ data, selected }: NodeProps) => {
  ...
  return (
    <Tooltip
      content={
        <div className={cn(
          ...
          <div className="font-bold text-xs">{nodeData.label}</div>
          <div className="text-[10px]">{subtitleMap[nodeData.subType] || 'Data Source'}</div>
          {nodeResult ? (
            <div>
              <div className="font-semibold text-xs mt-1">Engagement: {nodeResult.scores.engagement}</div>
    ...
    >
      ...
      >
        <div className="p-1 bg-emerald-500/20 rounded-xl">
